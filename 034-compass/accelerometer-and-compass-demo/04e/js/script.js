if(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)){
  ...
  document.getElementById("accelPermsButton").innerHTML = "Request device motion use.[disabled]"
}
...
function getAccel(){
    DeviceMotionEvent.requestPermission().then(response => {
       ...
       window.addEventListener('deviceorientation',(event) => {
		...
		// take alpha rotation degrees and convert to compass direction letters
		var direction = degToCompass(rotation_degrees)
		
		// report the value back
		document.getElementById('direction_value').innerHTML = "direction = " + direction + " derived from <br />" + rotation_degrees;
		   
		// report just the raw CompassHeading value
		document.getElementById('compassdir_value').innerHTML = compassdir;
		   
		   (fieldname4-fieldname3 >= 30 && fieldname4-fieldname3 <= 60)
		   
		if (compassdir >= 260 && compassdir <= 300) {
          document.getElementById('lake').style.opacity = "1.0";
        }
...
window.navigator.geolocation.getCurrentPosition(function(pos) {
  ...
  document.getElementById('angleToLake').innerHTML =  angleToLake + "&deg;";
	
  // $(".mobile").css('color', '#bbb');
});
